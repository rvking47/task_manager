...
import { Circle, Clock, icons, TrendingUp, Zap } from 'lucide-react'
    ...
    const fetchTasks= useCallback(async ()=>{
        ...
        try{
           ...
           if(!token) throw new Error("No auth token found")
            ...
            const {data} = await axios.get("https://task-manager-g67m.onrender.com/api/task/gp",{
                        ...
                        {tasks.slice(0,3).map((task)=>(
                            <div key={task._id || task.id} className='flex items-center justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-purple-100'>
                                <div className='flex-1 min-w-0'>
                                     <p className='text-sm font-medium text-gray-700 break-words whitespace-nowrap'>
                                      {task.title}
                                     </p>
