...
import { CalendarCheck, ChevronDown, LogOut, Settings } from 'lucide-react';
           ...
           <div ref={menuref} className="relative">
             <button onClick={handleMenuToggle} className="flex items-center gap-2 px-3 py-2 rounded-full cursor-pointer hover:bg-purple-50 transition-colors duration-300 border border-transparent hover:border-purple-200">
                 <div className="relative">
                   {user.avatar ?(
                    <img src={user.avatar} alt="Avtar" className="w-9 h-9 rounded-full shadow-sm" />
                   ):(
             ...
             {menuOpen && (
                <ul className="absolute top-14 right-0 w-56 bg-white rounded-2xl shadow-xl border border-purple-100 z-50 overflow-hidden animate-fadeIn">
                    <li className="p-2">
                       ...
                       <li className="p-2">
                         <button onClick={handleLogout} className="flex w-full item-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-red-50 text-red-600">
